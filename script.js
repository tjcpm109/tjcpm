...
async function syncToSheet(record) {
  ...
  try {
    const payload = stripLocalFields(record);   // ← 新增：剔除 _pendingSync 再送出
    const cbName = `cb_` + Date.now() + `_` + Math.random().toString(36).slice(2);
    const url = GAS_URL + `?callback=` + cbName + `&data=` + encodeURIComponent(JSON.stringify(payload)); // ← record 改成 payload
...
/*async function handleAction(type) {
  ...
  if (window.punchInFlight) return;              // 8.23← 新增：防連點
  window.punchInFlight = true;
...
async function handleAction(type) {
  if (!currentUser) return;
  if (!isGpsLocked) { showToast(`⚠️ 無法取得 GPS 定位，請允許位置存取後再試`); return; }
  if (window.punchInFlight) return;              // ← 新增：防連點
  window.punchInFlight = true;
  ...
  const { synced, queued } = await createAndSyncRecord(record, {
    onSuccessMsg: type === `上班` ? `✅ 上班打卡成功` : `👋 下班打卡成功`
  });

  if (synced) {
    addNotif(`system`, `${type}打卡成功 ${record.time} 📍`);
  } else if (queued) {
    addNotif(`system`, `${type}打卡已記錄 ${record.time} 📍 (待同步)`);
    if (!isOnline) showToast(`✅ 離線${type}打卡成功！已加入補傳佇列`);
  }

  window.punchInFlight = false;
}
async function processOfflineQueue() {
  ...
  for (let i = 0; i < offlineQueue.length; i++) {
    ...
    try {
            const payload = stripLocalFields(record);          // ← 新增：剔除 _pendingSync
      await fetch(GAS_URL, {
        method: `POST`,
        mode: `no-cors`,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)                     // ← record 改成 payload // body: JSON.stringify(record)
      });
      addNotif(`system`, `☁️ 離線打卡 (${record.date} ${record.time} ${record.type}) 已成功自動補傳同步！`);
    // ← 新增：補送成功後，清掉 records 裡對應那筆的 _pendingSync
      const rec = records.find(r => String(r.clientId || r.id) === String(record.clientId || record.id));
      if (rec) { delete rec._pendingSync; }
    } catch (err) {
      remaining.push(record);
    }
  }
  saveRecords();   // ← 新增：迴圈跑完後統一存一次（原本 records 陣列有被動到，要記得存檔）
...
async function submitLeave() {
  ...
  const { synced } = await createAndSyncRecord(record, { onSuccessMsg: `📝 請假申請已送出` });
...
async function submitOvertime() {
  ...
  //8.23records.unshift(record);
  //8.23saveRecords();
  //8.23renderAllList();
const { synced } = await createAndSyncRecord(record, { onSuccessMsg: `🌙 加班申請已送出` });  // ← 取代原本 unshift~syncToSheet 那 6 行
  ...
  //8.23await syncToSheet(record);
  //8.23addNotif(`pending`, `加班申請已送出（${record.date}），等待主管審核`);
  //8.23showToast(`🌙 加班申請已送出`);
  if (synced) addNotif(`pending`, `加班申請已送出（${record.date}），等待主管審核`);  // ← 加 if (synced)
}
...
async function submitAdjust() {
  ...
  //saveRecords();
  //renderAllList();
const { synced } = await createAndSyncRecord(record, { onSuccessMsg: `⚙️ 班別調整申請已送出` });

  if (filesToUpload.length > 0) {
    showToast(`📤 證明文件上傳中…`);
    await uploadProofFiles(clientId, filesToUpload, `班別調整`);
  }
   if (synced) addNotif(`pending`, `班別調整申請已送出（${record.date} ${record.subType}），等待主管審核`); 
  //await syncToSheet(record);
...
async function submitAdminAdjust() {
  ...
  const { synced } = await createAndSyncRecord(record, { onSuccessMsg: `✓ 班別異動指派成功並已直接生效！` });
...
async function quickLeave() {
  ...
  const record = {
    ...
    const { synced } = await createAndSyncRecord(record, { onSuccessMsg: `📝 請假申請已送出` });  // ← 取代 unshift~syncToShee
  //records.unshift(record);
  //saveRecords();
  //renderAllList();
  //syncToSheet(record);
...
async function quickOvertime() {
  ...
  const { synced } = await createAndSyncRecord(record, { onSuccessMsg: `🌙 加班申請已送出` });
  ...
  closeModal(`overtime`);
  //showToast(`🌙 加班申請已送出`);
  if (synced) addNotif(`pending`, `加班申請已送出（${record.date} ${record.hours}），等待主管審核`);
}
...
async function syncProfileAndAccumulatedLeaves() {
  try {
    ...
    if (data.status === `ok` && data.updates) {
      ...
      records = mergeUserRecords(records, data.updates, currentUser.empId);   // ← 改這兩行 //records = [...keepRecords, ...data.updates];
      saveRecords();
      ...
      calcAttendance();
      localStorage.setItem(`tjcpm_recordsLastSyncTime`, new Date().toISOString());  // ← 新增
      return { ok: true, changed: true };
    }
    localStorage.setItem(`tjcpm_recordsLastSyncTime`, new Date().toISOString());     // ← 新增（沒有 updates 也算成功）
    return { ok: true, changed: false };
  } catch (e) {
    return { ok: false, changed: false };   // ← 失敗不寫時間戳，讓下次可以馬上重試
  }
}
