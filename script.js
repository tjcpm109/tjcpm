...
// 全域目前篩選狀態變數 (統一命名與管理；唯一宣告點，勿在檔案其他地方重複宣告)
...
// ═══════════════════════════════════════════════════════════════
// ── 假別額度：資料同步層 vs 渲染層（合併後不再互相搶寫 DOM） ──
//
// 分工原則：
//  1) applyMyStatusData()：只負責把後端回傳的 records / quota 等資料
//     寫進 currentUser、records、localStorage（資料落地），
//     不在這裡直接操作任何額度相關 DOM。
//  2) updateLeaveBalanceDisplay()：唯一負責畫面渲染的函式，
//     所有跟「假別餘額 / 已用時數 / 審核中 / 到期提醒」有關的
//     DOM（leaveAnnualBalance、leaveCompBalance、accumAnnual、
//     accumComp、accumSick...等）都只在這裡寫一次。
//  3) 資料同步完成後，由 applyMyStatusData() 主動呼叫
//     updateLeaveBalanceDisplay() 觸發畫面更新，兩者不再各自
//     計算、各自寫同一批 DOM。
// ═══════════════════════════════════════════════════════════════
...
function calculatePendingLeaveHours(subType) {
  ...
  records.forEach(r => {
    ...
    const cleanSubType = r.subType === '加班補休' ? '補休' : r.subType;   // ← 新增這行
    // 2. 條件檢查：工號符合 + 類型為請假 + 假別符合 + 狀態屬於審核中
    if (!matchEmpId(r.empId, currentUser.empId) || 
        r.type !== '請假' || 
        cleanSubType !== subType ||     // ← 改用正規化後的值比對 
...
function updateLeaveBalanceDisplay() {
  ...
  if (currentUser.joinDate) {
    ...
    if (win && win.start) {
      anniversaryStartStr = `${win.start.getFullYear()}-${String(win.start.getMonth() + 1).padStart(2, '0')}-${String(win.start.getDate()).padStart(2, '0')}`;
    }
  }
  ...
  if (Array.isArray(records)) {
    records.forEach(r => {
      if (r.type === '請假' && isFinalApproved(r.status) && r.date && (!anniversaryStartStr || r.date >= anniversaryStartStr)) {
        ...
        const hours = parseFloat(r.hours) || calculateLeaveHoursLocal(r, currentUser?.holidayStrings || []);
  ...
  // 填入 DOM 表格
  const setAccum = (id, val) => {
    const el = document.getElementById(id);
    if (el) el.textContent = `${parseFloat(val.toFixed(1))}h`;
  };

  setAccum('accumSick', sickUsed);
  setAccum('accumPersonal', personalUsed);
  setAccum('accumOfficial', officialUsed);
  setAccum('accumMarriage', marriageUsed);
  setAccum('accumFuneral', funeralUsed);
